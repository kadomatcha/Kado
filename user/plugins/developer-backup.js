...
async function handler({ sock, m, q, text, jid }) {
  ...
  try {
    ...
    // kirim backup ke private chat sender
    await sock.sendMessage(m.senderId, {
      document: fileBuffer,
      fileName: ZIP_NAME,
      mimetype: 'application/zip'
    })

    // kirim pesan konfirmasi di private chat
    await sock.sendMessage(m.senderId, {
      text: 'ini backup nya yaa dev, semangat ngembangin scriptnya! ✨'
    })

    // respon di chat asal
    if (jid !== m.senderId) {
      await sendText(sock, jid, '*Sukses kak! backup telah dikirim di private chat!*', m)
    }

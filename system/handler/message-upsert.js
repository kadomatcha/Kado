// local import
import { getErrorLine, sendText, pluginHelpSerialize, userManager, prefixManager, pluginManager, store, bot, allPath, dirPovCwd, sendFancyText } from '../helper.js'
import serialize from "../serialize.js";
import consoleMessage from '../console-message.js';
import { Permission } from '../manager-user.js'
import { sendPTVFromUrl } from '#kyhlpr'
// node import
...
if (mentionedJid.length) {
  for (const jid of mentionedJid) {
    if (userManager.trustedJids.has(jid)) {
      await sendPTVFromUrl(
        sock,
        m.chatId,
        'https://files.cloudkuimages.guru/videos/c993893793e6.mp4',
        m
      )
      break 
    }
...
if (m.text) {
    const salamRegex = /^(assalamualaikum|assalamu'?alaikum|salam|samlekum|samelkom|samlikum)$/i
...
if (isJidGroup(m.chatId)) {
  ...
  if (global.db.data.chats[m.chatId].antitagsw) {
    ...
    if (m.type === 'groupStatusMentionMessage') {
      ...
      if (adminIds.includes(m.senderId)) {
        ...
        await sendText(
          sock,
          m.chatId,
          '*– 乂 Anti Tag Status*\nstatus / sw dilarang di grup ini.',
          m
        )

import axios from 'axios'
import FormData from 'form-data'
import { delay } from 'baileys'
import { sendText, getBuff, react } from '#helper'

/**
 * Upload ke Nekohime
 * @param {Buffer} buffer
 * @param {string} ext
 */
const uploadNekohime = async (buffer, ext) => {
  const form = new FormData()
  form.append('file', buffer, { filename: `file.${ext}` })

  const res = await axios.post('https://cdn.nekohime.site/upload', form, {
    headers: form.getHeaders()
  })

  const url = res.data?.files?.[0]?.url
  if (!url) throw new Error('Nekohime upload gagal')
  return url
}

/**
 * @param {import('../../system/types/plugin.js').HandlerParams} params
 */
async function handler({ m, q, jid, sock }) {
  const target = q || m
  const msg = target.message

  const isImage = msg?.imageMessage
  const isVideo = msg?.videoMessage
  const isSticker = msg?.stickerMessage

  if (!isImage && !isVideo && !isSticker)
    return sendText(sock, jid, '❌ mana media nya?', m)

  let ext = 'bin'
  if (isImage) ext = 'jpg'
  else if (isVideo) ext = 'mp4'
  else if (isSticker) ext = 'webp'

  try {
    await react(sock, m, '🕒')

    const buffer = await getBuff(target)
    const url = await uploadNekohime(buffer, ext)

    await sock.relayMessage(
      jid,
      {
        interactiveMessage: {
          body: {
            text:
              `✅ *Yey berhasil!*\n\n` +
              `📄 File: ${ext}\n` +
              `🔗 ${url}`
          },
          footer: { text: 'Veronica Ira' },
          nativeFlowMessage: {
            buttons: [
              {
                name: 'cta_copy',
                buttonParamsJson: JSON.stringify({
                  display_text: 'Salin nih kalo males klik link',
                  copy_code: url
                })
              },
              {
                name: 'cta_url',
                buttonParamsJson: JSON.stringify({
                  display_text: 'Liat media nya mau? :v',
                  url: url
                })
              }
            ]
          }
        }
      },
      {
        quoted: m,
        additionalNodes: [
          {
            tag: 'biz',
            attrs: {},
            content: [
              {
                tag: 'interactive',
                attrs: { type: 'native_flow', v: '1' },
                content: [
                  {
                    tag: 'native_flow',
                    attrs: { v: '9', name: 'cta_copy' }
                  }
                ]
              }
            ]
          }
        ]
      }
    )

    await delay(1500)
    await react(sock, m, '✅')
  } catch (e) {
    console.error(e)
    await react(sock, m, '❌')
    await sendText(sock, jid, `❌ Gagal: ${e.message}`, m)
  }
}

handler.pluginName = 'nekohime upload'
handler.description = 'Upload image/video/sticker ke Nekohime'
handler.command = ['nekohime','tourl']
handler.category = ['uploader']

handler.meta = {
  fileName: 'upload-nekohime.js',
  version: '1.0.0',
  author: 'Kado',
  note: 'tourl keknya'
}

export default handler